...
(
    ...
    observacao     text,
    status         varchar(30)    not null default 'PEDENTE',
    check ( status in ('PENDENTE', 'EM_PREPARO', 'PRONTO', 'ENTREGUE', 'CANCELADO') )
);

create index idx_pedido_itens_pedido on pedido_item (pedido_id);
create index idx_pedido_itens_produto on pedido_item (produto_id);
create index idx_pedido_itens_status on pedido_item (status);
...
(
    ...
    criado_em                timestamp      not null default current_timestamp,
    check ( forma_pagamento in ('DINHEIRO', 'CARTAO_CREDITO', 'CARTAO_DEBITO') ),
    CHECK ( status IN ('PEDENTE', 'APROVADO', 'RECUSADO', 'CANCELADO') )
);
